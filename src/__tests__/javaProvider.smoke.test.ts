...
import { handleViaOrchestrator, buildModels } from "../../dist/javaProvider.js";
...
describe("stub TeaVM orchestrator — callable from node", () => {
  it("returns a canned JSON decision using the given response text", async () => {
    ...
    const d = await handleViaOrchestrator(inputs, config);
  ...
  it("buildModels(3) returns the three-model catalog", () => {
    expect(buildModels(3)).toEqual({
